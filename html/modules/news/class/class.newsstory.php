...
class NewsStory extends XoopsStory
{
	...
	*/
	...
	function prepare2show($filescount)
	{
	    ...
	    $story['text'] = $this->phpBBsmiley($this->hometext());	// by yoshis
...
//        	$morelink .= ' | '.sprintf(_NW_BYTESMORE,$totalcount); // by makinosuke
		...
		if($infotips>0) {
			$story['infotips'] = ' title="'.make_infotips($this->hometext()).'"';
		} else {
			$story['infotips'] = '';
		}
	...
	* @return	string
	*/
	function phpBBsmiley($message){	// by yoshis
		$message = str_replace(":download:", "<img src=".XOOPS_URL."/modules/news/images/attachment.gif alt='' />", $message);
		return $message;
	}
}
?>
