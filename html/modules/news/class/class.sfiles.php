...
class sFiles
{
	...
	function getFileByDownloadName($dname)
	{
		$sql = "SELECT * FROM " . $this->table . " WHERE downloadname='" . basename($dname) . "'";
		$array = $this->db->fetchArray($this->db->query($sql));
		$this->makeFile($array);
	}
	...
	function setSize($value) // by yoshis
	{
		$this->size = $value;
	}
	...
	function getRealNameByDownloadName($dname)
	{
		$this->getFileByDownloadName($dname);
		return $this->getFileRealName();
	}
	...
	function getSize($format = "S")
	{ // by yoshis
		$myts =& MyTextSanitizer::getInstance();
		switch ($format) {
			case "S":
			case "Show":
				$size = $myts->htmlSpecialChars($this->size);
				break;
		}
		return $size;
	}
