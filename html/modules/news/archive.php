<?php
// $Id: archive.php,v 1.5 2006/09/28 17:04:50 yoshis Exp $
//  ------------------------------------------------------------------------ //
...
$tomonth = 0; $month_no_assignment = (isset($_GET['month'])) ? 0 : 1;//by makinosuke @ 2008/3/25
$pgtitle='';
if($fromyear && $frommonth) {
	$pgtitle=sprintf("%dǯ%d��",$fromyear,$frommonth);//by makinosuke @2008/3/25
}
...
$xoopsTpl->assign('xoops_pagetitle', $myts->htmlSpecialChars(_NW_NEWSARCHIVES)." ".$pgtitle . ' | ' . $myts->htmlSpecialChars($xoopsModule->name()));//by makinosuke @2008/3/25
	...
	while (list($time) = $xoopsDB->fetchRow($result)) {
		$time = formatTimestamp($time, "mysql", $useroffset);
			...
			if ($lastyear != $this_year) {
				...
				$years[$i]['months'] = SortMonths($months);
				$months = array();
				$lastmonth = 0;
				$lastyear = $this_year;
				$i++;
...
// In case of less the month, show whole year news.
...
if ($fromyear != 0 && $frommonth != 0) {
	...
	if($month_no_assignment == 1){ $xoopsTpl->assign('currentmonth', ''); }else{ $xoopsTpl->assign('currentmonth', $months_arr[$frommonth]); } // by makinosuke @2008/03/22-19:51
	...
	$monthend = mktime(23 - $timeoffset, 59, 59, $frommonth + 1 + $tomonth, 0, $fromyear);
	...
	if(is_array($storyarray) && $count>0) {
		foreach ($storyarray as $article) {
			...
			$htmltitle='';
			if($infotips>0) {
				$story['infotips'] = make_infotips($article->hometext());
...
function SortMonths($months){
	...
	foreach($months as $key => $row){
	    $MonOrder[$key] = $row['number'];
	}
	array_multisort($MonOrder,SORT_ASC,$months);
	return $months;
}
